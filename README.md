...
当前代码版本：`v0.0.4`（2026-07-17）。完整变更见 [CHANGELOG.md](CHANGELOG.md)。

## v0.0.4 已实现

- 每日自动取号，跨天显示 A、B … AA 前缀
- 主窗口与桌面悬浮窗共用同一人工队列
- 单击事项即复制 800×1200 竖版取号卡片
- 行内仅保留复制、上移、下移三个操作
- 右键查看、编辑、改状态、加急、完成、归档或移入回收站
- 部门 / 团队、对接人、事项类型、状态、优先级和截止时间
- 本地 SQLite 数据库、结构化状态历史和加急历史
- 事项类型和经办对接人支持可输入下拉选择，并自动记忆已保存内容
- 表单下方显示三个最常用对接人快捷项
- “关于”页面展示作者六朝生、GitHub 开源地址和当前软件版本
...
4. 安装并启动 v0.0.4；程序会先备份旧数据库，再在事务中执行迁移。
5. 在主界面左下角确认显示 `v0.0.4`，再核对原有事项、号码和日志。
