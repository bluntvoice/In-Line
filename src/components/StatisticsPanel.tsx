...
import { statisticsPresetRange,statisticsWeekdayLabel,type StatisticsPreset,type WeekStart } from "../lib/statistics-range";
  ...
  return <section className="statistics-panel">
    ...
    {loading?<div className="statistics-loading"><BarChart3 size={32}/><p>正在汇总处理活动…</p></div>:data&&<>
      ...
      <div className="statistics-grid"><section className="stat-card"><div className="section-heading"><div><h2>事项类型分布</h2><small>点击类型查看当前周期明细</small></div></div>{data.byTaskType.length?<div className="type-stats">{data.byTaskType.map(item=><button key={item.taskType} className={selectedType===item.taskType?"active":""} onClick={()=>void openType(item.taskType)}><span><strong>{item.taskType}</strong><small>已完成 {item.completed} · 待跟进 {item.pendingFollowUp}</small></span><b>{item.handledTasks}</b><ChevronRight size={17}/></button>)}</div>:<p className="empty-copy">当前周期没有有效处理事项。</p>}</section>
        <section className="stat-card trend-card"><div className="section-heading"><div><h2>工作量趋势</h2><small>{data.trendGranularity==="day"?"按日去重":"按自然周去重"}</small></div></div>{data.trend.length?<div className="trend-chart">{data.trend.map(point=><div className="trend-column" key={point.periodStart} title={`${point.periodStart}（${statisticsWeekdayLabel(point.periodStart)}）：${point.handledTasks} 项`}><b>{point.handledTasks}</b><span style={{height:`${Math.max(8,point.handledTasks/maxTrend*140)}px`}}/><small><span>{statisticsWeekdayLabel(point.periodStart)}</span><time dateTime={point.periodStart}>{point.periodStart.slice(5)}</time></small></div>)}</div>:<p className="empty-copy">暂无趋势数据。</p>}<p className="trend-note">趋势数据按每日或每周分别去重；同一事项可能在不同日期或不同周重复出现。顶部总数在整个查询周期内统一去重。</p></section>
      </div>
