...
export default function SettingsPanel({backups,settings,onChanged,notify}:{backups:BackupInfo[];settings:Record<string,string>;onChanged:()=>void;notify:(text:string)=>void}){
  ...
  const saveRateMode=async(value:"closure"|"processing")=>{const previous=rateMode;setRateMode(value);try{await api.setSetting("statistics_rate_mode",value);}catch(error){setRateMode(previous);notify("设置保存失败："+String(error));}};
