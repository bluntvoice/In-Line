...
export default function MultiContactInput({ values, options, commonContacts = [], itemLabel = "对接人", placeholder, onChange, onDelete }: Props) {
  ...
  return <div className="multi-contact" ref={root}>
    <div className="multi-contact-control" onClick={() => root.current?.querySelector("input")?.focus()}>
      {selected.map(name => <span className="contact-chip" key={name}>{name}<button type="button" aria-label={`移除${itemLabel}“${name}”`} onClick={event => { event.stopPropagation(); remove(name); }}><X size={13} /></button></span>)}
      <input value={draft} placeholder={selected.length ? "继续添加" : placeholder ?? `输入或选择${itemLabel}`} role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={listId}
      ...
      <button type="button" className="multi-contact-toggle" aria-label={`展开${itemLabel}选项`} onPointerDown={event => event.preventDefault()} onClick={event => { event.stopPropagation(); setOpen(current => !current); }}><ChevronDown size={16} /></button>
    </div>
