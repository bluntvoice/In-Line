...
import { addDays,calendarRange,dateKey,formatCalendarDuration,monthGrid,shiftCalendar,sortCalendarTasks,startOfDay,visibleWeekDays,type CalendarSort,type CalendarView,type WeekStart } from "../lib/work-calendar";
...
export default function WorkCalendarPanel({weekStartsOn,refreshKey,onOpenTask,notify}:{weekStartsOn:WeekStart;refreshKey:string|number;onOpenTask:(id:number)=>void;notify:(message:string)=>void}){
  ...
  const days=useMemo(()=>data&&view==="week"?visibleWeekDays(data,range.start):[],[data,view,range.start]);
  ...
  const rangeLabel=view==="week"?`${range.start.getFullYear()}-${String(range.start.getMonth()+1).padStart(2,"0")}-${String(range.start.getDate()).padStart(2,"0")} 至 ${dateKey(addDays(range.end,-1))}`:`${range.start.getFullYear()}年${range.start.getMonth()+1}月`;
  ...
  return <section className="work-calendar-panel">
    <header className="work-calendar-header">
      ...
      <div className="calendar-controls">
        <div className="calendar-nav"><button title={view==="week"?"上一周":"上一月"} onClick={()=>setAnchor(current=>shiftCalendar(current,view,-1))}><ChevronLeft size={17}/></button><button onClick={()=>setAnchor(new Date())}><RotateCcw size={15}/>今天</button><button title={view==="week"?"下一周":"下一月"} onClick={()=>setAnchor(current=>shiftCalendar(current,view,1))}><ChevronRight size={17}/></button></div>
        <div className="calendar-view-tabs"><button className={view==="week"?"active":""} onClick={()=>{setView("week");setSelectedDay(null);}}>周</button><button className={view==="month"?"active":""} onClick={()=>setView("month")}>月</button></div>
        <strong>{rangeLabel}</strong>
      </div>
    </header>
    {data&&<div className="calendar-summary"><span>{view==="week"?"本周":"本月"}</span><b>{data.summary.handledTasks}</b> 个事项<i/>处理 <b>{data.summary.handlingRounds}</b> 轮<i/>完成 <b>{data.summary.completedTasks}</b> 项</div>}
    {loading?<div className="calendar-state"><CalendarDays size={32}/><span>正在整理实际工作轨迹…</span></div>:error?<div className="calendar-state error"><span>{error}</span><button className="button secondary" onClick={()=>setAnchor(current=>new Date(current))}>重新载入</button></div>:data&&view==="week"?<>
      <div className="calendar-toolbar"><label>排序<select value={sort} onChange={event=>setSort(event.target.value as CalendarSort)}><option value="firstEnqueued">本周首次进入队列时间</option><option value="latestHandled">最近处理时间</option><option value="rounds">本周处理轮次（高到低）</option><option value="taskType">事项类型</option><option value="permanentNumber">永久编号</option></select></label><span><i className="legend-line"/>队列区间<i className="legend-dot handled"/>已处理<i className="legend-dot completed"/>已完成</span></div>
      <div className="calendar-week-scroll">
        <div className="calendar-week-grid" style={{minWidth:`${300+days.length*150}px`}}>
          <div className="calendar-week-corner">事项</div>
          <div className="calendar-day-header" style={{gridTemplateColumns:`repeat(${days.length},minmax(150px,1fr))`}}>{days.map(day=><div key={dateKey(day)}><strong>{WEEKDAYS[day.getDay()]}</strong><time>{String(day.getMonth()+1).padStart(2,"0")}-{String(day.getDate()).padStart(2,"0")}</time></div>)}</div>
          {tasks.map(task=><div className="calendar-week-row" key={task.taskId}>
            <button className="calendar-task-cell" onClick={()=>onOpenTask(task.taskId)} title={`${task.permanentNumber} · ${task.title}\n${task.taskType}`}><b>{task.permanentNumber}</b><strong>{task.title}</strong><span>{task.taskType}</span></button>
