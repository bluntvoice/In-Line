...
export default function App(){
  ...
  const openView=(next:PageView)=>{setSettings(false);setAbout(false);setStatistics(false);setSelected(null);setView(next);};
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>
        <button className={!settings&&!about&&!statistics&&view==="queue"?"active":""} onClick={()=>openView("queue")}><Inbox size={18}/><span>待办队列</span><b>{queueCount}</b></button>
        <button className={!settings&&!about&&!statistics&&view==="deferred"?"active":""} onClick={()=>openView("deferred")}><PauseCircle size={18}/><span>暂缓事项</span><span className="nav-counts"><b>{deferred.length}</b>{deferredOverdue>0&&<em title={`${deferredOverdue} 项已逾期`}><ClockAlert size={12}/>{deferredOverdue}</em>}</span></button>
        <button className={!settings&&!about&&!statistics&&view==="archive"?"active":""} onClick={()=>openView("archive")}><Archive size={18}/><span>历史归档</span><b>{data.archive.length}</b></button>
        <button className={statistics?"active":""} onClick={()=>{setStatistics(true);setSettings(false);setAbout(false);setSelected(null);}}><BarChart3 size={18}/><span>统计中心</span></button>
