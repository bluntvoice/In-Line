...
import { statisticsWeekdayLabel } from "./statistics-range";
...
function drawTrend(context:CanvasRenderingContext2D,x:number,y:number,width:number,height:number,options:StatisticsExportData){const rows=options.data.trend;cardFrame(context,x,y,width,height,"工作量趋势",options.data.trendGranularity==="day"?"按日去重":"按自然周去重");if(!rows.length){fillText(context,"暂无趋势数据。",x+24,y+122,13,COLORS.muted);return;}const chartX=x+36,chartY=y+105,chartWidth=width-72,chartHeight=height-174,max=Math.max(1,...rows.map(item=>item.handledTasks));context.strokeStyle=COLORS.line;context.beginPath();context.moveTo(chartX,chartY+chartHeight);context.lineTo(chartX+chartWidth,chartY+chartHeight);context.stroke();rows.forEach((item,index)=>{const {barWidth,barX,labelX}=trendBarLayout(chartWidth,rows.length,index);context.textAlign="center";if(item.handledTasks>0){const barHeight=Math.max(5,chartHeight*item.handledTasks/max);context.fillStyle=COLORS.blue;roundedRect(context,chartX+barX,chartY+chartHeight-barHeight,barWidth,barHeight,Math.min(5,barWidth/2));context.fill();fillText(context,String(item.handledTasks),chartX+labelX,chartY+chartHeight-barHeight-7,10,COLORS.ink,600);}fillText(context,statisticsWeekdayLabel(item.periodStart),chartX+labelX,chartY+chartHeight+18,9,COLORS.ink,600);fillText(context,item.periodStart.slice(5),chartX+labelX,chartY+chartHeight+33,9,COLORS.muted);context.textAlign="left";});}
